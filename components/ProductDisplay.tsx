...
export default function ProductDisplay({ limit = 4 }: { limit?: number }) {
  ...
  if (!products || products.length === 0) {
    return <div className="text-gray-500 dark:text-gray-400 text-center py-8">No trending products found.</div>;
  }
      ...
      {products.map((product) => (
        ...
        >
          ...
          <div className="relative h-64 overflow-hidden">
            ...
            >
              <button className="w-8 h-8 rounded-full bg-white text-gray-800 flex items-center justify-center shadow-md hover:bg-green-500 hover:text-white transition-colors">
                <Heart size={16} />
              </button>
              <Link 
          ...
          <div className="p-4">
            <div className="flex items-center mb-1">
              ...
              <span className="text-xs text-gray-500 dark:text-gray-400">({product.number_of_reviews})</span>
            </div>
            ...
            <div className="flex items-center justify-between mt-2">
              <div className="flex items-center">
                <span className="text-lg font-bold text-gray-900 dark:text-white">${product.final_price.toFixed(2)}</span>
                {product.discount > 0 && (
                  <span className="text-sm text-gray-500 dark:text-gray-400 line-through ml-2">${product.price.toFixed(2)}</span>
                )}
              </div>
